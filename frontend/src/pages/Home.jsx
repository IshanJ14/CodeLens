import { useState } from "react";
import api from "../services/api";
import SearchBar from "../components/SearchBar";
import ProfileCard from "../components/ProfileCard";
import NavBar from "../components/NavBar";
import { Container, Typography } from "@mui/material";
import RatingChart from "../components/RatingChart";
import AnalyticsCard from "../components/AnalyticsCards";
import { calculateContestStats } from "../utils/analytics";
import ContestHistoryTable from "../components/ContestHistoryTable";
import LoadingSkeleton from "../components/LoadingSkeleton";
import EmptyState from "../components/EmptyState";
import TagAnalysisChart from "../components/TagAnalysisChart";

function Home() {
    const [user, setUser] = useState(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState("");
    const [ratingHistory, setRatingHistory] = useState([]);
    const [tags, setTags] = useState({});

    const searchUser = async (handle) => {
        setLoading(true);
        setError(""); // Clear previous errors
        setUser(null);
        setRatingHistory([]); // Clear previous rating history
        setTags({});
        
        try {
            const [userResponse, ratingResponse, tagsResponse] = await Promise.all([
              api.get(`/users/${handle}`),
              api.get(`/users/${handle}/rating`),
              api.get(`/users/${handle}/tags`),
            ]);

            setUser(userResponse.data);
            setRatingHistory(ratingResponse.data);
            setTags(tagsResponse.data)
        } catch (error) {
            setTags({});
            if (error.response) {
              setError(error.response.data.detail);
            } else {
              setError("Something went wrong.");
            }
            //console.error(error);
        } finally {
            setLoading(false);
        }
    };

    const stats = 
        user && ratingHistory.length > 0 
          ? calculateContestStats(ratingHistory, user) : null;

  return (
    <>
      <NavBar />

      <Container maxWidth="md" sx={{ ml: 0, mr: 'auto', mt: 2 }}>

        <Typography 
          variant="h4" 
          //align="center" 
          gutterBottom
          fontWeight="bold"
        >
          Competitive Programming Analytics
        </Typography>

        <Typography
          //align="center"
          color="text.secondary"
          sx={{ mb: 4 }}
        >
          Search any Codeforces user to get their profile and contest statistics.
        </Typography>

        <SearchBar onSearch={searchUser} />

        {loading && (
          <LoadingSkeleton />
        )}

        {error && (
          <Typography color="error" sx={{ mt: 3 }}>
            {error}
          </Typography>
        )}
        
        {user && (
          <ProfileCard user={user} />
        )}

        {stats && (
          <AnalyticsCard stats={stats} />
        )}

        {user && ratingHistory.length === 0 && !loading && (
          <EmptyState />
        )}
        
        {ratingHistory.length > 0 && (
          <ContestHistoryTable history={ratingHistory} />
        )}

        {Object.keys(tags).length > 0 && (
          <TagAnalysisChart tags={tags} />
        )}
        
        {ratingHistory.length > 0 && (
          <RatingChart data={ratingHistory} />
        )}
      </Container>
    </>
  );
}

export default Home;